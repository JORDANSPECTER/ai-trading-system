...
# HARD LOCK — this generator can only ever create QQQ.
TICKER = "QQQ"
if TICKER != "QQQ":
...
LOOP_SECONDS = int(os.getenv("AUTO_SIGNAL_STANDALONE_LOOP_SECONDS", "15"))
...
def get_vwap():
    q = get_market()
    price = get_price()
    vwap = safe_float(q.get("vwap") or q.get("session_vwap"))
    if vwap and vwap > 100:
        return vwap
    # Fallback prevents VWAP-missing blocks during API outages.
    fallback = safe_float(q.get("close") or q.get("last") or price)
    if fallback and fallback > 100:
        print(f"[AUTO SIGNAL VWAP FALLBACK] using price/close as temporary VWAP={fallback}", flush=True)
        return fallback
    return None
...
def get_premarket_levels():
    ...
    pm_high = safe_float(q.get("premarket_high") or q.get("pre_market_high") or q.get("pm_high"))
    pm_low = safe_float(q.get("premarket_low") or q.get("pre_market_low") or q.get("pm_low"))

    if pm_high and pm_low:
        return pm_high, pm_low

    # Fallback keeps strategy context alive when pre/post data is blocked by provider plan.
    last = safe_float(q.get("last") or q.get("price") or q.get("close"))
    if last and last > 100:
        fallback_range = float(os.getenv("MARKET_DATA_PREMARKET_FALLBACK_RANGE", "2.50"))
        pm_high = round(last + fallback_range, 2)
        pm_low = round(last - fallback_range, 2)
        print(f"[AUTO SIGNAL PM FALLBACK] high={pm_high} low={pm_low}", flush=True)
        return pm_high, pm_low
...
def get_oil_bias():
    ...
    if not isinstance(oil, dict):
        oil = {}
    oil_text = str(oil.get("trend") or oil.get("direction") or oil.get("state") or macro.get("oil_trend", "")).lower()
...
def score_trade(price, vwap, levels, oil_bias, pm_high, pm_low):
    ...
    if session_name() in {"open", "power_hour"}:
        call_score += 1
        put_score += 1
    ...
    return {"direction": direction, "score": score, "grade": "A+" if score >= 6 else "A", "setup": setup, "reasons": reasons}
...
def build_signal(price, vwap, decision, levels, oil_bias, pm_high, pm_low):
    ...
    signal = {
        "ticker": "QQQ", "symbol": "QQQ", "underlying": "QQQ",
        "direction": direction, "side": direction,
        "entry": round(price, 2), "entry_price": round(price, 2),
        "stop": stop, "stop_loss": stop, "targets": targets, "target": targets[0],
        "confidence": decision["grade"], "grade": decision["grade"], "score": decision["score"],
        "setup": decision["setup"], "setup_type": "premarket_vwap_levels_oil", "trigger": "premarket_level_entry_type",
        "vwap": round(vwap, 2), "oil_bias": oil_bias,
        "premarket_high": pm_high, "premarket_low": pm_low, "pm_high": pm_high, "pm_low": pm_low,
        "level_context": levels, "strategy_reasons": decision["reasons"],
        "session": session_name(), "time_window": session_name(),
        "trade_reason": "QQQ setup generated from premarket levels, VWAP, key levels, oil/macro bias, and anti-chasing filter.",
        "timestamp": now_ts(), "created_at": now_iso(), "source": "auto_signal_generator",
        "force_execution": True, "paper_only": True,
    }
...
def generate_signal(force=False):
    ...
    if not decision:
        print(
            f"[AUTO SIGNAL] no trade | ticker=QQQ price={round(price, 2)} vwap={round(vwap, 2)} "
    ...
    print(
        f"[AUTO SIGNAL] Generated QQQ {signal['direction']} setup={signal['setup']} grade={signal['grade']} "
        f"score={signal['score']} entry={signal['entry']} vwap={signal['vwap']} pm_high={pm_high} pm_low={pm_low}",
        flush=True,
    )
    return signal
...
if __name__ == "__main__":
    print("[AUTO SIGNAL LOOP STARTED] QQQ ONLY", flush=True)
    while True:
        try:
            result = generate_signal(force=False)
            if result:
