...
print("[MARKET REGIME ENGINE ACTIVE] realtime trend/chop/expansion/event classifier enabled", flush=True)
...
# =========================================================
# LIVE TRADE EXIT PROTECTION ENGINE
# Real-time protective exits layered on top of Step 12/13 management.
# =========================================================
ENABLE_LIVE_EXIT_PROTECTION = os.getenv("ENABLE_LIVE_EXIT_PROTECTION", "true").lower() == "true"
EXIT_PROTECT_BREAKEVEN_TRIGGER_PCT = float(os.getenv("EXIT_PROTECT_BREAKEVEN_TRIGGER_PCT", "0.12"))
EXIT_PROTECT_BREAKEVEN_OFFSET_PCT = float(os.getenv("EXIT_PROTECT_BREAKEVEN_OFFSET_PCT", "0.01"))
EXIT_PROTECT_PARTIAL_TP_TRIGGER_PCT = float(os.getenv("EXIT_PROTECT_PARTIAL_TP_TRIGGER_PCT", "0.20"))
EXIT_PROTECT_PARTIAL_TP_QTY_PCT = float(os.getenv("EXIT_PROTECT_PARTIAL_TP_QTY_PCT", "0.50"))
EXIT_PROTECT_TRAIL_TRIGGER_PCT = float(os.getenv("EXIT_PROTECT_TRAIL_TRIGGER_PCT", "0.25"))
EXIT_PROTECT_TRAIL_GIVEBACK_PCT = float(os.getenv("EXIT_PROTECT_TRAIL_GIVEBACK_PCT", "0.35"))
EXIT_PROTECT_HARD_PROFIT_LOCK_TRIGGER_PCT = float(os.getenv("EXIT_PROTECT_HARD_PROFIT_LOCK_TRIGGER_PCT", "0.40"))
EXIT_PROTECT_HARD_PROFIT_LOCK_PCT = float(os.getenv("EXIT_PROTECT_HARD_PROFIT_LOCK_PCT", "0.20"))
EXIT_PROTECT_MAX_HOLD_SECONDS = int(float(os.getenv("EXIT_PROTECT_MAX_HOLD_SECONDS", "1800")))
EXIT_PROTECT_STALL_SECONDS = int(float(os.getenv("EXIT_PROTECT_STALL_SECONDS", "420")))
EXIT_PROTECT_STALL_MIN_MOVE_PCT = float(os.getenv("EXIT_PROTECT_STALL_MIN_MOVE_PCT", "0.06"))
EXIT_PROTECT_CLOSE_ON_VWAP_FAIL = os.getenv("EXIT_PROTECT_CLOSE_ON_VWAP_FAIL", "true").lower() == "true"
EXIT_PROTECT_VWAP_FAIL_BUFFER_PCT = float(os.getenv("EXIT_PROTECT_VWAP_FAIL_BUFFER_PCT", "0.0015"))
EXIT_PROTECT_CLOSE_ON_OIL_FLIP = os.getenv("EXIT_PROTECT_CLOSE_ON_OIL_FLIP", "true").lower() == "true"
EXIT_PROTECT_OIL_FLIP_DOLLARS = float(os.getenv("EXIT_PROTECT_OIL_FLIP_DOLLARS", "0.75"))
EXIT_PROTECT_CLOSE_ON_WIDE_SPREAD = os.getenv("EXIT_PROTECT_CLOSE_ON_WIDE_SPREAD", "true").lower() == "true"
EXIT_PROTECT_MAX_SPREAD_PCT = float(os.getenv("EXIT_PROTECT_MAX_SPREAD_PCT", "0.30"))
EXIT_PROTECT_ALERTS = os.getenv("EXIT_PROTECT_ALERTS", "true").lower() == "true"
EXIT_PROTECT_EVENT_FILE = os.getenv("EXIT_PROTECT_EVENT_FILE", "live_exit_protection_events.jsonl").strip()
...
def select_institutional_qqq_option_contract(signal: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for c in contracts:
        symbol = normalize_option_symbol(c)
    ...
    for contract in candidates:
        ...
        if ok:
            if best is None:
                ...
                new_dist = abs(strike - price)
                old_mid = safe_float(best_details.get("contract_mid"), 999.0)
                new_mid = contract_mid if contract_mid > 0 else 999.0
                if (new_spread_pct, new_dist, new_mid) < (old_spread_pct, old_dist, old_mid):
                    best, best_details = contract, details
        else:
...
# LIVE TRADE EXIT PROTECTION ENGINE
# =========================================================
def exit_protection_event(event: str, payload: Dict[str, Any]) -> None:
    try:
        row = {"time": now_ts(), "ts": epoch(), "event": event, **(payload or {})}
        with open(EXIT_PROTECT_EVENT_FILE, "a", encoding="utf-8") as f:
            f.write(json.dumps(row, default=str) + "\n")
    except Exception:
        pass


def exit_protection_alert(position: Dict[str, Any], title: str, reason: str, extra: Optional[Dict[str, Any]] = None) -> None:
    if not EXIT_PROTECT_ALERTS:
        return
    try:
        symbol = str(position.get("symbol", "UNKNOWN"))
        pnl = round(safe_float(position.get("pnl_pct", 0), 0), 2)
        msg = (
            f"🛡️ {title}\n"
            f"Symbol: {symbol}\n"
            f"Reason: {reason}\n"
            f"PnL: {pnl}%\n"
            f"Qty Open: {position.get('qty_open')}\n"
            f"Mode: {position.get('mode')}\n"
            f"⏰ {now_ts()}"
        )
        if extra:
...
def infer_underlying_from_position(position: Dict[str, Any]) -> str:
    for key in ("ticker", "underlying", "underlying_symbol", "root_symbol"):
        val = str(position.get(key, "")).upper().strip()
        if val:
            return val
    symbol = str(position.get("symbol", "")).upper().strip()
    for root in ("QQQ", "SPY", "SPX", "IWM", "DIA", "NVDA", "TSLA", "AAPL", "MSFT", "META", "AMD"):
        if symbol.startswith(root):
            return root
    return "QQQ" if "QQQ" in symbol else symbol[:3]


def get_underlying_market_snapshot(position: Dict[str, Any]) -> Dict[str, Any]:
    try:
        underlying = infer_underlying_from_position(position)
        prices = load_market_prices([underlying, "USO"])
        snap = prices.get(underlying, {}) if isinstance(prices, dict) else {}
...
def get_vwap_from_snapshot(snapshot: Dict[str, Any]) -> float:
    for key in ("vwap", "VWAP", "session_vwap", "anchored_vwap"):
        v = safe_float(snapshot.get(key, 0), 0) if isinstance(snapshot, dict) else 0
        if v > 0:
            return v
    return 0.0


def oil_flip_against_position(position: Dict[str, Any]) -> Tuple[bool, str, Dict[str, Any]]:
    if not EXIT_PROTECT_CLOSE_ON_OIL_FLIP:
        return False, "oil_flip_disabled", {}
    try:
        prices = load_market_prices(["USO"])
        uso = prices.get("USO", {}) if isinstance(prices, dict) else {}
        if not isinstance(uso, dict):
            return False, "no_uso_snapshot", {}
        current = safe_float(uso.get("price", uso.get("last", 0)), 0)
        prev = safe_float(uso.get("prev_close", uso.get("previous_close", uso.get("open", 0))), 0)
        change = safe_float(uso.get("change", 0), 0)
        if change == 0 and current > 0 and prev > 0:
            change = current - prev
        direction = str(position.get("direction", position.get("side", "CALL"))).upper()
        if "CALL" in direction or str(position.get("bias", "")).upper() == "BULLISH":
            if change >= EXIT_PROTECT_OIL_FLIP_DOLLARS:
                return True, "oil_rising_against_calls", {"USO_change": round(change, 3), "USO_price": current}
        if "PUT" in direction or str(position.get("bias", "")).upper() == "BEARISH":
            if change <= -EXIT_PROTECT_OIL_FLIP_DOLLARS:
                return True, "oil_falling_against_puts", {"USO_change": round(change, 3), "USO_price": current}
        return False, "oil_ok", {"USO_change": round(change, 3), "USO_price": current}
    except Exception as e:
        return False, f"oil_check_error:{e}", {}


def option_spread_too_wide(position: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
    if not EXIT_PROTECT_CLOSE_ON_WIDE_SPREAD:
        return False, {}
    bid = safe_float(position.get("bid", position.get("last_bid", 0)), 0)
    ask = safe_float(position.get("ask", position.get("last_ask", 0)), 0)
    mid = safe_float(position.get("mid", position.get("mark", 0)), 0)
    if mid <= 0 and bid > 0 and ask > 0:
        mid = (bid + ask) / 2
    if bid <= 0 or ask <= 0 or mid <= 0:
        return False, {"spread_check": "missing_bid_ask"}
    spread_pct = (ask - bid) / mid if mid > 0 else 0
    return spread_pct >= EXIT_PROTECT_MAX_SPREAD_PCT, {"bid": bid, "ask": ask, "mid": round(mid, 4), "spread_pct": round(spread_pct, 4)}


def close_position_with_exit_protection(position: Dict[str, Any], live_price: float, reason: str, extra: Optional[Dict[str, Any]] = None) -> bool:
    try:
        position.setdefault("exit_protection_reasons", []).append({"time": now_ts(), "reason": reason, "extra": extra or {}})
        exit_protection_event("exit_triggered", {"symbol": position.get("symbol"), "reason": reason, "extra": extra or {}, "mode": position.get("mode")})
        exit_protection_alert(position, "LIVE EXIT PROTECTION TRIGGERED", reason, extra)
        if position.get("mode") == "LIVE":
            return bool(live_close_position(position, f"LIVE EXIT PROTECTION - {reason}"))
        finalize_close_position(position, live_price, f"LIVE EXIT PROTECTION - {reason}")
        return True
    except Exception as e:
        log(f"❌ EXIT PROTECTION CLOSE ERROR | {position.get('symbol')} | {reason} | {e}")
        exit_protection_event("exit_close_error", {"symbol": position.get("symbol"), "reason": reason, "error": str(e)})
        return False


def live_trade_exit_protection_engine(position: Dict[str, Any], live_price: float) -> Tuple[bool, bool]:
    """Returns (changed, closed). Protective layer before normal TP/stop logic."""
    if not ENABLE_LIVE_EXIT_PROTECTION:
        return False, False
    try:
        entry = safe_float(position.get("avg_fill_price", 0), 0) or safe_float(position.get("entry_price", 0), 0)
        qty_open = safe_int(position.get("qty_open", 0), 0)
        if entry <= 0 or live_price <= 0 or qty_open <= 0:
            return False, False
        changed = False
        move_pct = (live_price - entry) / entry
        opened_at = safe_int(position.get("opened_at", position.get("open_time", epoch())), epoch())
        age = max(0, epoch() - opened_at)
        highest = safe_float(position.get("highest_price", live_price), live_price)
        current_stop = safe_float(position.get("stop_price", 0), 0)

        if move_pct >= EXIT_PROTECT_BREAKEVEN_TRIGGER_PCT and not position.get("exit_protect_breakeven_set", False):
            be_stop = round(entry * (1 + EXIT_PROTECT_BREAKEVEN_OFFSET_PCT), 4)
            if be_stop > current_stop:
                position["stop_price"] = be_stop
                position["exit_protect_breakeven_set"] = True
                position.setdefault("notes", []).append(f"Exit protection breakeven: {current_stop} -> {be_stop}")
                exit_protection_event("breakeven_set", {"symbol": position.get("symbol"), "old_stop": current_stop, "new_stop": be_stop, "move_pct": round(move_pct, 4)})
                exit_protection_alert(position, "BREAKEVEN PROTECTION SET", "trade moved in favor", {"new_stop": be_stop, "move_pct": round(move_pct * 100, 2)})
                changed = True

        if AUTO_TP_ENABLED and move_pct >= EXIT_PROTECT_PARTIAL_TP_TRIGGER_PCT and not position.get("exit_protect_partial_taken", False) and not position.get("tp1_hit", False):
            qty_to_close = max(1, math.floor(qty_open * EXIT_PROTECT_PARTIAL_TP_QTY_PCT))
            qty_to_close = min(qty_to_close, qty_open)
            ok = live_scale_out(position, qty_to_close, "EXIT PROTECTION PARTIAL TAKE PROFIT") if position.get("mode") == "LIVE" else scale_out_local(position, qty_to_close, live_price, "EXIT PROTECTION PARTIAL TAKE PROFIT")
            if ok:
                position["exit_protect_partial_taken"] = True
                position["tp1_hit"] = True
                position["stop_price"] = max(safe_float(position.get("stop_price", 0), 0), entry)
                exit_protection_event("partial_profit_taken", {"symbol": position.get("symbol"), "qty": qty_to_close, "price": live_price, "move_pct": round(move_pct, 4)})
                exit_protection_alert(position, "PARTIAL PROFIT TAKEN", "exit protection partial", {"qty": qty_to_close, "price": live_price})
                changed = True
                if safe_int(position.get("qty_open", 0), 0) <= 0:
                    return True, True

        if move_pct >= EXIT_PROTECT_TRAIL_TRIGGER_PCT and highest > entry:
            profit_open = max(0, highest - entry)
            trail_stop = round(highest - (profit_open * EXIT_PROTECT_TRAIL_GIVEBACK_PCT), 4)
            if trail_stop > safe_float(position.get("stop_price", 0), 0):
                old_stop = safe_float(position.get("stop_price", 0), 0)
                position["stop_price"] = trail_stop
                position.setdefault("notes", []).append(f"Exit protection trail: {old_stop} -> {trail_stop}")
                exit_protection_event("trail_raised", {"symbol": position.get("symbol"), "old_stop": old_stop, "new_stop": trail_stop, "highest": highest})
                changed = True

        if move_pct >= EXIT_PROTECT_HARD_PROFIT_LOCK_TRIGGER_PCT:
            lock_stop = round(entry * (1 + EXIT_PROTECT_HARD_PROFIT_LOCK_PCT), 4)
            if lock_stop > safe_float(position.get("stop_price", 0), 0):
                old_stop = safe_float(position.get("stop_price", 0), 0)
                position["stop_price"] = lock_stop
                position.setdefault("notes", []).append(f"Exit protection hard profit lock: {old_stop} -> {lock_stop}")
                exit_protection_event("hard_profit_lock", {"symbol": position.get("symbol"), "old_stop": old_stop, "new_stop": lock_stop})
                changed = True

        if EXIT_PROTECT_CLOSE_ON_VWAP_FAIL:
            snap = get_underlying_market_snapshot(position)
            underlying_price = safe_float(snap.get("price", snap.get("last", 0)), 0) if isinstance(snap, dict) else 0
            vwap = get_vwap_from_snapshot(snap)
            direction = str(position.get("direction", position.get("side", "CALL"))).upper()
            if underlying_price > 0 and vwap > 0:
                buffer = vwap * EXIT_PROTECT_VWAP_FAIL_BUFFER_PCT
                if ("CALL" in direction or str(position.get("bias", "")).upper() == "BULLISH") and underlying_price < (vwap - buffer):
                    ok = close_position_with_exit_protection(position, live_price, "VWAP failed against call thesis", {"underlying_price": underlying_price, "vwap": vwap})
                    return True, ok
                if ("PUT" in direction or str(position.get("bias", "")).upper() == "BEARISH") and underlying_price > (vwap + buffer):
                    ok = close_position_with_exit_protection(position, live_price, "VWAP reclaimed against put thesis", {"underlying_price": underlying_price, "vwap": vwap})
                    return True, ok

        oil_bad, oil_reason, oil_extra = oil_flip_against_position(position)
        if oil_bad:
            ok = close_position_with_exit_protection(position, live_price, oil_reason, oil_extra)
            return True, ok

        spread_bad, spread_extra = option_spread_too_wide(position)
        if spread_bad:
            ok = close_position_with_exit_protection(position, live_price, "option spread too wide", spread_extra)
            return True, ok

        if age >= EXIT_PROTECT_MAX_HOLD_SECONDS:
            ok = close_position_with_exit_protection(position, live_price, "max hold time reached", {"age_seconds": age})
            return True, ok
        if age >= EXIT_PROTECT_STALL_SECONDS and abs(move_pct) < EXIT_PROTECT_STALL_MIN_MOVE_PCT:
            ok = close_position_with_exit_protection(position, live_price, "trade stalled with weak movement", {"age_seconds": age, "move_pct": round(move_pct, 4)})
            return True, ok

        return changed, False
    except Exception as e:
        log(f"❌ LIVE EXIT PROTECTION ERROR | {position.get('symbol')} | {e}")
        exit_protection_event("engine_error", {"symbol": position.get("symbol"), "error": str(e)})
        return False, False
...
def manage_open_positions(incoming_signal: Optional[Dict[str, Any]] = None):
    ...
    for position in list(open_positions):
        ...
        # LIVE EXIT PROTECTION runs after Step 13 and before normal TP/stop logic.
        exit_changed, exit_closed = live_trade_exit_protection_engine(position, live_price)
        if exit_changed:
            changed = True
        if exit_closed:
            continue
...
def boot():
    ...
    if ENABLE_MACRO_BRIDGE:
        ...
        try:
            ...
            pass
    try:
        market_regime_tick(force=True)
    except Exception as e:
        try: debug(f"MARKET REGIME BOOT ERROR | {e}")
        except Exception: pass
...
# =========================================================
# MARKET REGIME ENGINE — REAL-TIME BRAIN LAYER
# Classifies current conditions before entries and feeds existing
# evaluate_regime/adaptive-risk layers with a clean regime signal.
# =========================================================
ENABLE_MARKET_REGIME_ENGINE_V2 = os.getenv("ENABLE_MARKET_REGIME_ENGINE_V2", os.getenv("ENABLE_MARKET_REGIME_ENGINE", "true")).lower() == "true"
MARKET_REGIME_FILE = os.getenv("MARKET_REGIME_FILE", "market_regime_state.json").strip()
MARKET_REGIME_EVENT_LOG_FILE = os.getenv("MARKET_REGIME_EVENT_LOG_FILE", "market_regime_events.jsonl").strip()
...
MARKET_REGIME_MAX_DATA_AGE_SECONDS = int(float(os.getenv("MARKET_REGIME_MAX_DATA_AGE_SECONDS", str(MARKET_DATA_STALE_SECONDS if 'MARKET_DATA_STALE_SECONDS' in globals() else 20))))
MARKET_REGIME_SEND_ALERTS = os.getenv("MARKET_REGIME_SEND_ALERTS", "true").lower() == "true"
MARKET_REGIME_ALERT_ON_CHANGE_ONLY = os.getenv("MARKET_REGIME_ALERT_ON_CHANGE_ONLY", "true").lower() == "true"
MARKET_REGIME_RANGE_TIGHT_PCT = float(os.getenv("MARKET_REGIME_RANGE_TIGHT_PCT", "0.0018"))
MARKET_REGIME_RANGE_EXPANSION_PCT = float(os.getenv("MARKET_REGIME_RANGE_EXPANSION_PCT", "0.0060"))
MARKET_REGIME_VWAP_DISTANCE_TREND_PCT = float(os.getenv("MARKET_REGIME_VWAP_DISTANCE_TREND_PCT", "0.0015"))
MARKET_REGIME_OIL_EVENT_MOVE_PCT = float(os.getenv("MARKET_REGIME_OIL_EVENT_MOVE_PCT", "0.018"))
MARKET_REGIME_SCORE_TREND_MIN = float(os.getenv("MARKET_REGIME_SCORE_TREND_MIN", "0.65"))
MARKET_REGIME_SCORE_EXPANSION_MIN = float(os.getenv("MARKET_REGIME_SCORE_EXPANSION_MIN", "0.58"))
MARKET_REGIME_SCORE_NORMAL_MIN = float(os.getenv("MARKET_REGIME_SCORE_NORMAL_MIN", "0.45"))
...
def _mr_float(v, default: float = 0.0) -> float:
    try:
        if v is None or v == "":
            return default
        return float(v)
...
def _mr_load_market_prices() -> Dict[str, Any]:
    paths = []
    try:
        paths.append(MARKET_DATA_FILE)
    except Exception:
        pass
    try:
        paths.append(MARKET_PRICES_FILE)
    except Exception:
        pass
    paths.append("market_prices.json")
    for p in paths:
        try:
...
def _mr_pick_symbol_snapshot(data: Dict[str, Any], symbol: str) -> Dict[str, Any]:
    symbol = str(symbol or "").upper().strip()
    if not isinstance(data, dict) or not symbol:
        return {}
    candidates = [
        data.get(symbol),
        data.get(symbol.lower()),
        (data.get("symbols", {}) or {}).get(symbol) if isinstance(data.get("symbols", {}), dict) else None,
        (data.get("prices", {}) or {}).get(symbol) if isinstance(data.get("prices", {}), dict) else None,
        (data.get("market_prices", {}) or {}).get(symbol) if isinstance(data.get("market_prices", {}), dict) else None,
        (data.get("data", {}) or {}).get(symbol) if isinstance(data.get("data", {}), dict) else None,
    ]
    for c in candidates:
        if isinstance(c, dict) and c:
            return c
    if symbol == MARKET_REGIME_PRIMARY_SYMBOL and any(k in data for k in ["price", "last", "vwap", "high", "low"]):
        return data
    return {}


def _mr_get_price_fields(snap: Dict[str, Any]) -> Dict[str, Any]:
    price = _mr_float(snap.get("price", snap.get("last", snap.get("close", snap.get("last_price", 0)))), 0)
    vwap = _mr_float(snap.get("vwap", snap.get("VWAP", snap.get("session_vwap", price))), price)
    high = _mr_float(snap.get("high", snap.get("day_high", snap.get("premarket_high", price))), price)
    low = _mr_float(snap.get("low", snap.get("day_low", snap.get("premarket_low", price))), price)
    open_px = _mr_float(snap.get("open", snap.get("day_open", price)), price)
    prev_close = _mr_float(snap.get("prev_close", snap.get("previous_close", snap.get("prior_close", open_px))), open_px)
    ts = 0
    for k in ["updated_ts", "timestamp", "ts", "price_updated_ts", "updated_at_ts", "last_update_ts"]:
        ts = int(_mr_float(snap.get(k), 0))
        if ts > 0:
            break
    return {"price": price, "vwap": vwap, "high": high, "low": low, "open": open_px, "prev_close": prev_close, "ts": ts}


def _mr_data_fresh(fields: Dict[str, Any]) -> Tuple[bool, str]:
    ts = int(_mr_float(fields.get("ts"), 0))
    if ts <= 0:
        if MARKET_REGIME_REQUIRE_FRESH_DATA:
            return False, "missing_market_timestamp"
        return True, "timestamp_missing_allowed"
    age = max(0, _mr_now_ts() - ts)
    if age > MARKET_REGIME_MAX_DATA_AGE_SECONDS:
        return False, f"stale_market_data_age_{age}s"
    return True, f"fresh_age_{age}s"


def market_regime_log_event(event: str, payload: Dict[str, Any]) -> None:
    try:
        row = {"time": _mr_iso(), "ts": _mr_now_ts(), "event": event, **(payload or {})}
        with open(MARKET_REGIME_EVENT_LOG_FILE, "a", encoding="utf-8") as f:
            f.write(json.dumps(row, default=str) + "\n")
    except Exception:
        pass


def market_regime_detect(force: bool = False) -> Dict[str, Any]:
    if not ENABLE_MARKET_REGIME_ENGINE_V2 and not force:
        return {"enabled": False, "regime": "NORMAL", "score": 0.50, "reason": "disabled"}
    data = _mr_load_market_prices()
    primary_snap = _mr_pick_symbol_snapshot(data, MARKET_REGIME_PRIMARY_SYMBOL)
    q = _mr_get_price_fields(primary_snap)
    fresh_ok, fresh_reason = _mr_data_fresh(q)
    price = q["price"]
    vwap = q["vwap"] or price
    high = max(q["high"], price)
    low = min(q["low"], price) if q["low"] else price
    open_px = q["open"] or price
    prev_close = q["prev_close"] or open_px or price

    if price <= 0:
        regime = "NORMAL"
        score = 0.40
        reasons = ["missing_primary_price"]
    else:
        range_pct = abs(high - low) / max(price, 0.01)
        vwap_dist_pct = abs(price - vwap) / max(price, 0.01)
        open_move_pct = abs(price - open_px) / max(open_px, 0.01) if open_px > 0 else 0
        prev_move_pct = abs(price - prev_close) / max(prev_close, 0.01) if prev_close > 0 else 0
        above_vwap = price >= vwap
        directional_score = min(1.0, (vwap_dist_pct / max(MARKET_REGIME_VWAP_DISTANCE_TREND_PCT, 0.0001)) * 0.45 + (open_move_pct / 0.006) * 0.30 + (prev_move_pct / 0.008) * 0.25)
        expansion_score = min(1.0, (range_pct / max(MARKET_REGIME_RANGE_EXPANSION_PCT, 0.0001)) * 0.70 + (vwap_dist_pct / 0.003) * 0.30)
        chop_score = min(1.0, max(0.0, (MARKET_REGIME_RANGE_TIGHT_PCT / max(range_pct, 0.00001))) * 0.55 + max(0.0, (MARKET_REGIME_VWAP_DISTANCE_TREND_PCT / max(vwap_dist_pct, 0.00001))) * 0.45)
        oil_event = False
        oil_detail = ""
        oil_snap = _mr_pick_symbol_snapshot(data, "USO") or _mr_pick_symbol_snapshot(data, "CL") or _mr_pick_symbol_snapshot(data, "OIL")
        if oil_snap:
            oil = _mr_get_price_fields(oil_snap)
            oil_price = oil.get("price", 0)
            oil_prev = oil.get("prev_close", oil.get("open", 0))
            if oil_price and oil_prev:
                oil_move_pct = abs(oil_price - oil_prev) / max(oil_prev, 0.01)
                oil_event = oil_move_pct >= MARKET_REGIME_OIL_EVENT_MOVE_PCT
                oil_detail = f"oil_move_pct={round(oil_move_pct*100,2)}"
        reasons = [fresh_reason, f"range_pct={round(range_pct*100,3)}", f"vwap_dist_pct={round(vwap_dist_pct*100,3)}", f"above_vwap={above_vwap}"]
        if oil_detail:
            reasons.append(oil_detail)
        if oil_event:
            regime = "EVENT"
            score = max(0.35, min(0.70, expansion_score))
            reasons.append("oil_event_volatility")
        elif chop_score >= 0.72 and range_pct <= MARKET_REGIME_RANGE_TIGHT_PCT * 1.75:
            regime = "CHOP"
            score = max(0.20, min(0.44, 1.0 - chop_score / 2))
            reasons.append("tight_range_near_vwap")
        elif directional_score >= MARKET_REGIME_SCORE_TREND_MIN and vwap_dist_pct >= MARKET_REGIME_VWAP_DISTANCE_TREND_PCT:
            regime = "TREND"
            score = max(MARKET_REGIME_SCORE_TREND_MIN, min(0.95, directional_score))
            reasons.append("directional_vwap_control")
        elif expansion_score >= MARKET_REGIME_SCORE_EXPANSION_MIN:
            regime = "EXPANSION"
            score = max(MARKET_REGIME_SCORE_EXPANSION_MIN, min(0.90, expansion_score))
            reasons.append("range_expansion")
        else:
            regime = "NORMAL"
            score = max(MARKET_REGIME_SCORE_NORMAL_MIN, min(0.65, (directional_score + expansion_score) / 2))
            reasons.append("standard_conditions")
        if not fresh_ok and MARKET_REGIME_REQUIRE_FRESH_DATA:
            regime = "EVENT" if MARKET_REGIME_BLOCK_EVENT else "NORMAL"
            score = min(score, 0.40)
            reasons.append("fresh_data_required_failed")

    state = {
        "enabled": bool(ENABLE_MARKET_REGIME_ENGINE_V2),
        "generated_at": _mr_iso(),
        "generated_ts": _mr_now_ts(),
        "primary_symbol": MARKET_REGIME_PRIMARY_SYMBOL,
        "regime": regime,
        "market_regime": regime,
        "score": round(float(score), 4),
        "regime_score": round(float(score), 4),
        "fresh_data_ok": bool(fresh_ok),
        "fresh_data_reason": fresh_reason,
        "price": price,
        "vwap": vwap,
        "high": high,
        "low": low,
        ...
        "reasons": reasons,
        "blocking": {"block_chop": bool(MARKET_REGIME_BLOCK_CHOP), "block_event": bool(MARKET_REGIME_BLOCK_EVENT), "require_fresh_data": bool(MARKET_REGIME_REQUIRE_FRESH_DATA)},
    }
    try:
        atomic_write_json(MARKET_REGIME_FILE, state)
    except Exception:
        try:
            with open(MARKET_REGIME_FILE, "w", encoding="utf-8") as f:
...
def market_regime_format(state: Dict[str, Any]) -> str:
    r = str(state.get("regime", "NORMAL")).upper()
    icon = {"TREND": "📈", "EXPANSION": "⚡", "NORMAL": "✅", "CHOP": "⚠️", "EVENT": "🌎"}.get(r, "🧠")
    return (f"{icon} MARKET REGIME ENGINE\nRegime: {r} | Score: {state.get('score')}\n{state.get('primary_symbol')}: price={state.get('price')} vwap={state.get('vwap')} high={state.get('high')} low={state.get('low')}\nFresh Data: {state.get('fresh_data_ok')} | {state.get('fresh_data_reason')}\nWhy: {', '.join([str(x) for x in state.get('reasons', [])][:6])}\n⏰ {state.get('generated_at')}")


def market_regime_tick(force: bool = False) -> Dict[str, Any]:
    if not ENABLE_MARKET_REGIME_ENGINE_V2 and not force:
        return {"enabled": False, "regime": "NORMAL", "score": 0.50}
    try:
        ensure_globals_initialized()
    except Exception:
        pass
    try:
        last_ts = int((GLOBAL_STATE or {}).get("last_market_regime_ts", 0)) if isinstance(GLOBAL_STATE, dict) else 0
    except Exception:
        last_ts = 0
    if not force and (_mr_now_ts() - last_ts) < MARKET_REGIME_REFRESH_SECONDS:
        try:
            existing = load_json_file(MARKET_REGIME_FILE, {})
            if isinstance(existing, dict) and existing:
                return existing
        except Exception:
            return {"enabled": True, "regime": "NORMAL", "score": 0.50, "reason": "cooldown"}
    prev_regime = None
    try:
        prev = load_json_file(MARKET_REGIME_FILE, {})
        if isinstance(prev, dict):
            prev_regime = str(prev.get("regime", "")).upper()
    except Exception:
        pass
    state = market_regime_detect(force=force)
    try:
        GLOBAL_STATE["last_market_regime_ts"] = int(state.get("generated_ts", _mr_now_ts()))
        GLOBAL_STATE["market_regime"] = state.get("regime", "NORMAL")
        GLOBAL_STATE["market_regime_score"] = state.get("score", 0.5)
        save_state(GLOBAL_STATE)
    except Exception:
        pass
    try:
        market_regime_log_event("regime_detected", state)
    except Exception:
        pass
    changed = prev_regime and prev_regime != str(state.get("regime", "")).upper()
    if MARKET_REGIME_SEND_ALERTS and (force or changed or not MARKET_REGIME_ALERT_ON_CHANGE_ONLY):
        msg = market_regime_format(state)
        try:
            send_to_discord(DISCORD_AI_WEBHOOK, msg, "AI")
        except Exception:
            pass
...
def market_regime_apply_to_signal(signal: Dict[str, Any]) -> Dict[str, Any]:
    x = dict(signal or {})
    if not ENABLE_MARKET_REGIME_ENGINE_V2:
        return x
    try:
        state = market_regime_tick(force=False)
        regime = str(state.get("regime", "NORMAL")).upper()
        score = _mr_float(state.get("score", 0.5), 0.5)
        x["market_regime"] = regime
        x["regime"] = regime
        x["regime_score"] = score
        x["market_regime_detail"] = state
        if not isinstance(x.get("context"), dict):
            x["context"] = {}
        x["context"]["market_regime"] = regime
        x["context"]["market_regime_score"] = score
    except Exception as e:
        try:
            debug(f"MARKET REGIME APPLY ERROR | {e}")
        except Exception:
            pass
    return x


def market_regime_pretrade_gate(signal: Dict[str, Any]) -> Dict[str, Any]:
    if not ENABLE_MARKET_REGIME_ENGINE_V2:
        return {"approved": True, "reason": "disabled", "signal": signal}
    x = market_regime_apply_to_signal(signal)
    state = x.get("market_regime_detail", {}) if isinstance(x.get("market_regime_detail", {}), dict) else {}
    regime = str(x.get("market_regime") or state.get("regime") or "NORMAL").upper()
    score = _mr_float(x.get("regime_score", state.get("score", 0.5)), 0.5)
    reasons = []
    ...
    if not approved:
        try:
            msg = build_block_message("MARKET REGIME ENGINE BLOCKED SIGNAL", x, reasons, f"Regime: {regime} | Score: {score}")
        except Exception:
            msg = f"MARKET REGIME BLOCK | {regime} | {reasons}"
        try:
            send_to_discord(DISCORD_AI_WEBHOOK, msg, "AI")
            send_to_live_entry_discord(msg)
            send_to_telegram(msg)
            ...
            pass
        market_regime_log_event("signal_blocked", {"regime": regime, "score": score, "reasons": reasons, "ticker": x.get("ticker"), "direction": x.get("direction")})
    else:
        market_regime_log_event("signal_approved", {"regime": regime, "score": score, "ticker": x.get("ticker"), "direction": x.get("direction")})
    return {"approved": approved, "reasons": reasons, "regime": regime, "score": score, "state": state, "signal": x}
